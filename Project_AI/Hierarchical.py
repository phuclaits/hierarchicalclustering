...
class Hierarchical_Clustering:
    ...
    def euclidean_distance(self, data_point_one, data_point_two):
        ...
        result = 0.0
        for i in range(size):
            f1 = float(data_point_one[i])   # điểm 1
            f2 = float(data_point_two[i])   # điểm 2 
            tmp = f1 - f2
            result += pow(tmp, 2)
    ...
    def compute_pairwise_distance(self, dataset):
        ...
        for i in range(dataset_size-1):    
            for j in range(i+1, dataset_size):     # eclid 2 điểm  trong dataset 
                dist = self.euclidean_distance(dataset[i]["data"], dataset[j]["data"])
                result.append( (dist, [dist, [[i], [j]]]) )
        return result
                
    def build_priority_queue(self, distance_list): # hàng đợi check sự ưu tiên khi sắp xếp tăng dần 
        heapq.heapify(distance_list)
        self.heap = distance_list
        return self.heap

    def compute_centroid_two_clusters(self, current_clusters, data_points_index): # tính tâm khi có 2 clusters
    ...
    def compute_centroid(self, dataset, data_points_index): # tính khoảng cách của tâm
        size = len(data_points_index)
        dim = self.dimension
        centroid = [0.0]*dim
